...
- **`components/TacticalBoard.tsx`** — thin top-level wrapper; composes the toolbar, canvas, panel, and modals, plus the editable title/category/description header when `backHref` is passed (see Save flow below)
...
- **Inline metadata editing** (added 2026-07-08): title and category are directly editable in the header rendered by `TacticalBoard.tsx` (title as a plain-looking `<input>`, category as a styled `<select>`); description is behind a chevron toggle that reveals a textarea beneath the header. All three are read-only display (no input/select) when `viewOnly`. This doesn't autosave — it updates the same `title`/`description`/`category` state in `hooks/useTacticalBoard.ts` that the Save panel's Save tab and quick-save already read from, so editing inline then hitting Ctrl+S (or the panel's Save button) persists it, and editing in the panel updates the header live too — one shared source of truth, not two. The header itself only renders when `TacticalBoard` receives `backHref` (only `app/playbook/[id]/page.tsx` passes it; the player portal embed doesn't, since it has its own header).
